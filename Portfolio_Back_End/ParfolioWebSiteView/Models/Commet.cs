...
namespace ParfolioWebSiteView.Models
{
    public class Commet
    {
        ...
        public async Task<List<Commet>> ComentChildernAsync(IQueryable<Commet> soruce,int take,int id)
        {

            if (CommentChildren.Count > take) return CommentChildren;
            ...
            foreach (var item in coments)
            {
                if (item.IsChild)
                {
                    coments.InsertRange(coments.IndexOf(item) + 1,(IEnumerable<Commet>)ComentChildernAsync(soruce, take, item.Id));
                }
            }
